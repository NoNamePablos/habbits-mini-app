...
<template>
  <Card
    ...
    :class="achievement.unlocked ? 'bg-card' : 'bg-muted/50 opacity-60'"
  >
    <CardContent class="pt-4 pb-4 flex flex-col items-center text-center gap-2">
      <div
        class="w-12 h-12 rounded-full flex items-center justify-center"
        :class="achievement.unlocked ? 'bg-primary/20' : 'bg-muted'"
      >
        <component
          :is="achievement.unlocked && achievement.icon ? resolveIcon(achievement.icon) : Lock"
          class="h-5 w-5"
          :class="achievement.unlocked ? 'text-primary' : 'text-muted-foreground'"
        />
      </div>
      <div>
